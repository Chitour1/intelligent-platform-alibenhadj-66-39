...
import { ArrowLeft, Calendar, Clock, Share2, BookOpen, Video, ChevronDown, ChevronUp } from 'lucide-react';
...
const StatementDetails = () => {
  ...
  // State for timeline visibility
  const [timelineOpen, setTimelineOpen] = useState(false);

  // Predefined timeline for the March 22, 2025 video
  const videoTimeline = [
    { 
      id: 1, 
      startTime: "00:00:00", 
  ...
  // Helper function to jump to video time (you'll need to modify this based on your video embed method)
  const jumpToVideoTime = (timeStr: string) => {
    const videoElement = document.querySelector('iframe');
    if (videoElement) {
      const seconds = timeToSeconds(timeStr);
      const currentSrc = videoElement.src;
      const baseUrl = currentSrc.split('?')[0];
      videoElement.src = `${baseUrl}?start=${seconds}&autoplay=1`;
    }
  };
              ...
              <div className="mt-4 space-y-2 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                {videoTimeline.map((section) => (
                  <div 
                    ...
                    onClick={() => jumpToVideoTime(section.startTime)}
                  >
